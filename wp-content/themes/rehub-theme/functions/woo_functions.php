...
add_action('woocommerce_after_add_to_cart_button', 'rh_woo_add_custom_buttons_to_default');

function rh_woo_add_custom_buttons_to_default(){
	global $post;
	$postID = $post->ID;
	$rh_product_layout_style = get_post_meta($postID, '_rh_woo_product_layout', true);
	$output = '';
	if($rh_product_layout_style == '' || $rh_product_layout_style == 'default_full_width' || ($rh_product_layout_style == 'global' && rehub_option('product_layout_style') == 'default_full_width')){
		$output .= '<div class="button_action mobspt25 pl25 pr25 disabletabletpadding tabmt25">';
			$wishlistadded = esc_html__('Added to wishlist', 'rehub-theme');
			$wishlistremoved = esc_html__('Removed from wishlist', 'rehub-theme');
			$wishlist = RH_get_wishlist($postID, '', $wishlistadded, $wishlistremoved);
			if($wishlist){
				$output .= '<div class="floatleft mr15 def_btn rh-sq-icon-btn-big rh-flex-center-align rh-flex-justify-center">'.$wishlist.'</div>'; 
			}
			if(rehub_option('compare_page') || rehub_option('compare_multicats_textarea')){
				$output .= '<span class="compare_for_grid mr15 def_btn floatleft rh-sq-icon-btn-big rh-flex-center-align rh-flex-justify-center">';
				$cmp_btn_args = array(); 
				...
				$output .= wpsm_comparison_button($cmp_btn_args).'</span>';
			}  
